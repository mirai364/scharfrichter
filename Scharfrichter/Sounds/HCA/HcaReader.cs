    ...
    public abstract class HcaReader
    {
        ...
        internal void ParseHeaders()
        {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.HCA))
            {
                ...
                hcaInfo.Version = HcaStreamHelper.SwapEndian(header.Version);
                hcaInfo.DataOffset = HcaStreamHelper.SwapEndian(header.DataOffset);
            }
            else
            {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.FMT))
            {
                ...
                hcaInfo.SamplingRate = HcaStreamHelper.SwapEndian(header.SamplingRate << 8);
                hcaInfo.BlockCount = HcaStreamHelper.SwapEndian(header.Blocks);
                hcaInfo.FmtR01 = HcaStreamHelper.SwapEndian(header.R01);
                hcaInfo.FmtR02 = HcaStreamHelper.SwapEndian(header.R02);
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.COMP))
            {
                ...
                stream.Read(out header);
                hcaInfo.BlockSize = HcaStreamHelper.SwapEndian(header.BlockSize);
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.VBR))
            {
                VbrHeader header;
                stream.Read(out header);
                hcaInfo.VbrR01 = HcaStreamHelper.SwapEndian(header.R01);
                hcaInfo.VbrR02 = HcaStreamHelper.SwapEndian(header.R02);
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.LOOP))
            {
                LoopHeader header;
                stream.Read(out header);
                hcaInfo.LoopStart = HcaStreamHelper.SwapEndian(header.LoopStart);
                hcaInfo.LoopEnd = HcaStreamHelper.SwapEndian(header.LoopEnd);
                hcaInfo.LoopR01 = HcaStreamHelper.SwapEndian(header.R01);
                hcaInfo.LoopR02 = HcaStreamHelper.SwapEndian(header.R02);
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.CIPH))
            {
                CipherHeader header;
                stream.Read(out header);
                hcaInfo.CipherType = (CipherType)HcaStreamHelper.SwapEndian(header.Type);
            }
            else
            {
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.RVA))
            {
                RvaHeader header;
                stream.Read(out header);
                hcaInfo.RvaVolume = HcaStreamHelper.SwapEndian(header.Volume);
            }
            else
            {
                hcaInfo.RvaVolume = 1;
            ...
            if (MagicValues.IsMagicMatch(v, MagicValues.COMM))
            {
                ...
                List<byte> tmpCommentCharList = new List<byte>();
                byte tmpByte;
                do
                {
