...
namespace Scharfrichter.Tests
{
    ...
    public class ChuniPcParserTests
    {
        ...
        [Fact]
        public void SkipsRecognizedUnsupportedNewerTags()
        {
            // ASO (Air Solid), SLA (Soflan region), HHD/HHX (Heaven Hold) are
            // recognised C2S note types without a UGC v8 equivalent. They must
            // not hit the "undefined sign" branch or produce chart entries.
            var chart = Parse(
                "RESOLUTION\t384\n" +
                "SLA\t10\t0\t0\t4\t96\t1\n" +
                "ASO\t11\t0\t0\t4\t5.0\t3.0\t96\t2\t4\t5.0\t3.0\tDEF\n" +
                "HHD\t12\t0\t0\t4\t5.0\t96\t2\t4\t5.0\t0\n" +
                "HHX\t13\t0\t0\t4\t5.0\t96\t2\t4\t5.0\t0\tUP\n");

            // No marker / event entries should be generated by the skipped tags.
            var markers = chart.Entries.Where(e =>
                e.Type == EntryTypeChuni.Marker || e.Type == EntryTypeChuni.Event).ToList();
            Assert.Empty(markers);
        }

        [Fact]
        public void AirCrushWidthCollisionKeepsSeparateMetadata()
        {
            // Two ALD chains share the same (offset, column) but have different
            // widths. Their connection-point metadata (Height/EndHeight) must
            // stay on the correct chain instead of leaking to the other one.
            var chart = Parse(
                "RESOLUTION\t384\n" +
                "ALD\t0\t0\t0\t1\t0\t1.0\t192\t0\t1\t1.0\tYEL\n" +
                "ALD\t0\t0\t0\t2\t0\t1.0\t192\t0\t2\t1.0\tYEL\n" +
                "ALD\t0\t192\t0\t1\t0\t1.0\t7\t0\t1\t5.0\tYEL\n" +
                "ALD\t0\t192\t0\t2\t0\t1.0\t7\t0\t2\t9.0\tYEL\n");

            var connections = chart.Entries.Where(e =>
                e.Type == EntryTypeChuni.Marker && e.Player == 7 &&
                (int)((double)e.LinearOffset) == 192 && e.Column == 0).ToList();

            Assert.Equal(2, connections.Count);

            var width1 = connections.Single(e => e.Value.Numerator % 100 == 1);
            var width2 = connections.Single(e => e.Value.Numerator % 100 == 2);

            Assert.Equal(5.0, width1.EndHeight);
            Assert.Equal(9.0, width2.EndHeight);
        }

        [Fact]
        public void ParsesHardAirHold()
        {
            var chart = Parse("RESOLUTION\t384\nAHX\t42\t0\t4\t4\tTAP\t144\tDEF\n");
            var airHolds = chart.Entries.Where(e => e.Type == EntryTypeChuni.Marker && e.Player == 4).ToList();
            Assert.Equal(2, airHolds.Count); // start + end pair

            var start = airHolds.First(e => (int)(e.Value.Numerator / 100) == 1);
            var end = airHolds.First(e => (int)(e.Value.Numerator / 100) == 2);
            Assert.Equal(4, start.Column);
            // TAP companion is not a linked-ground code, so Parameter stays 0.
            Assert.Equal(0, start.Parameter);
            Assert.Equal("DEF", start.Tag);
            Assert.Equal(start.Identifier, end.Identifier);
        }
